...
{{--            @csrf--}}
            <div class="p-4 pt-3 flex-grow-1 position-relative mh-100dvh" data-trigger="scrollbar">
                ...
                @if($request->status == PENDING)
                    ...
                    <div class="mb-4 character-count status-deny{{$request->id}} d-none">
                        <textarea placeholder="Type a note about request denial" name="denied_note"
                                  rows="5" class="form-control character-count-field"  maxlength="1500" data-max-character="1500" disabled></textarea>
                        <span>{{translate('0/1500')}}</span>
                    </div>
                @endif
                @if($request->status == PENDING)
                    <h3 class="mb-3 text-center status-approve{{$request->id}} d-none">{{ translate("Approval Note") }}</h3>
                    <div class="mb-4 character-count status-approve{{$request->id}} d-none">
                        <textarea placeholder="Type a note about request approval"
                                  name="approval_note"
                                  rows="5" class="form-control character-count-field"  maxlength="1500" data-max-character="1500" disabled></textarea>
                        <span>{{translate('0/1500')}}</span>
                    </div>
